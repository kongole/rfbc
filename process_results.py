...
def compare_results():
    ...
    archive_dict = {entry["url"]: entry["content"].lower() for entry in archive_data}
    live_dict = {entry["url"]: entry["content"].lower() for entry in live_data}
    ...
    for url, old_text in archive_dict.items():
        ...
        # Skip URLs missing from the live data
        if not new_text:
            logging.warning(f"Skipping {url} - Not found in live results.")
            continue
...
if __name__ == "__main__":
    compare_results()
    print("Keyword analysis complete. Check 'removed_keywords.json'.")
